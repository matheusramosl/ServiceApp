...
function PackageItem({props}) {
  ...
  const {selectOptions, setPackageType} = useContext(context)
  const serviceList = [
   'Business Formation',
   '1099',
   'Annual Report USA',
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {/* <dialog id="my_modal_5" className="modal items-center justify-items-end ">
              <div className="modal-box h-4/6 ">
                ...
                {selectOptions.filter((i) => i.Type === 'service').length > 0 ? selectOptions.filter((i) => i.Type === 'package').map(({Name, Amount}) => <PackageItemModal props={{Name, Amount }} />) : <p className='ml-10'>No Package Selected</p>}
                  <button className="w-4 h-4 text-gray-500 hover:text-gray-700 focus:outline-none " onClick={() => setPackageType('package')}>X</button>
                </form>
              </div>
            </dialog> */}
        </div>
